...
## 三映CSV → 勘太郎CSV 変換（ルールベース・試作）

上記のAI読取とは別に、**三映CSV（CSV-A）を機械的に解釈して勘太郎向けCSV（CSV-B）を1案件1ファイルで生成する**変換エンジンを試作しています（雄進印刷様の実データで検証済み）。AIは使わず、変換ルールはすべて `sanei_config.yaml` に外出ししてあります（GAS版へ移す際はスプレッドシートへ転記）。

```bash
# 手元の三映CSV（顧客情報を含むためリポジトリには入れない）を変換
python tools/run_sanei_demo.py path/to/CSV-A.csv --out ./out
python tests/test_sanei_converter.py   # 合成データでの単体テスト
```

- `app/sanei_converter.py` … 変換エンジン（本番/校正セクション分割・裏行除外・受注№グルーピング・品名連結・用紙/印刷サイズ変換・色数分解・営業日計算・4パターンの備考文面）
- `sanei_config.yaml` … 列定義・固定値・変換表・備考テンプレート・祝日。コード変更なしで仕様変更に対応
- 出力形式（文字コード・囲み・ファイル名）は勘太郎の正式仕様書または実機テストで最終確定（現状は実見本の UTF-8 / 最小囲みを既定）
